...
void minimize_Topology::copymatvalue(double x, double y, int index)
{
    Target_Traj[index].coeffRef(0)=x;
    Target_Traj[index].coeffRef(1)=y;
    ...
    for(int steps=0;steps<decnum;steps++)
    {
        for(int i=0;i<agentnum;i++)
        {
            ...
            else
            {
                ...
                Eigen::Vector2d postarget=Target_Traj[steps];//就是这里需要改，需要将Target_Traj重新输入
