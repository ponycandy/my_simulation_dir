...
simulator_manager::simulator_manager(QObject *parent) : QObject(parent)
{
    ...
    SimDynamicsservice *SimDy_service_0=SwarmSimulator_201Activator::getService<SimDynamicsservice>("SimDynamicsservice");
    Colli_service=SwarmSimulator_201Activator::getService<CollisionDetectservice>("CollisionDetectservice");
    //碰撞检测，用以检测无人车是否进入多边形区域
    ...
    SwarmSimulator_201Activator::publishsignal(m_sim,SIGNAL(sig_agent_state_update(QMap<int,SwarmAgent*>)),OSGIEVENT::SIG_AGENT_STATE_UPDATE,Qt::QueuedConnection);

    qRegisterMetaType<QMap<int,SwarmObstacle*> >("QMap<int,SwarmObstacle*>");
    SwarmSimulator_201Activator::publishsignal(m_sim,SIGNAL(sig_obs_state_update(QMap<int,SwarmObstacle*>)),OSGIEVENT::SIG_OBS_STATE_UPDATE,Qt::QueuedConnection);
