...
double PotentialCalc(double z, double &first, double &second)
{
    ...
    //        if ((z >=0)  && (z < h))
    //        {
    //            output= 1; // Assign -1 if the number is negative
    //            first =0;
    //            second =0;
    //            return output;
    //        }
    ...
    //            if ((z >= h) && (z < 1))
    //            {
    //                output = 0.5*(1+cos(mathpi*(z-h)/(1-h))); //% Assign 0 if the number is between 0 and 10 (inclusive)
    //                first=0.5*mathpi/(h-1)*sin(mathpi*(z-h)/(1-h));
    //                second=-0.5*mathpi/pow((h-1),2)*cos(mathpi*(z-h)/(1-h));
    //                return output;
    //            }
    //            else
    //            {
